    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "Keep the Allocator Above 2GB", new SettingItem("General", "MimallocHighArena", false, null, "A 32-bit client can only allocate from the low 2GB, and this tool's allocator grows into the same half. Two testers ran out of it, and one had a SavedVariables file written under a garbage name. This reserves 512MB above 2GB and hands it to the allocator, which uses memory it is given before asking the OS. Needs a large-address-aware client on 64-bit Windows; it refuses to run if the block it gets is below 2GB. Size with MimallocHighArenaMB in wow_opt.ini. EXPERIMENTAL.", true) },
                { "Batch the Game's File Writes", new SettingItem("General", "ClientWriteBatch", false, null, "The game writes SavedVariables about nine bytes at a time. One tester's loading screen spent 2470 ms of 16828 inside 593557 of those calls, for 5.6 MB. This gathers them into 64KB pieces, so the same work is about ninety system calls. It buffers one file at a time, flushes on every close, seek, read and flush, and checks each closed file's size against what the game handed over - if a byte ever goes missing it switches itself off and says so. Needs File I/O Hooks. EXPERIMENTAL.", true) },
